...
func spawn_test_object():
	...
	if Engine.is_editor_hint() and tree.edited_scene_root:
		obj.owner = tree.edited_scene_root
		mesh_inst.owner = tree.edited_scene_root
	
	# Add Mover Script
	var mover_script = load("res://Development/Scripts/Systems/WaterSystem/Core/WaterTestMover.gd")
	...
	var interactor_script = load("res://Development/Scripts/Systems/WaterSystem/Components/WaterInteractor3D.gd")
	...
	if Engine.is_editor_hint() and tree.edited_scene_root:
		interactor.owner = tree.edited_scene_root
	
	# Find WaterManager
	var managers = tree.get_nodes_in_group("WaterSystem_Managers")
