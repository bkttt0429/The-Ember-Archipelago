...
@export var grid_res: int = 128:
	set(v):
		grid_res = v
		if is_inside_tree(): _request_restart()
...
@export var restart_simulation: bool = false:
	set(v): if is_inside_tree(): _request_restart()
...
const SOLVER_PATH = "res://Development/Scripts/Systems/WaterSystem/Core/Shaders/WaterSolver.glsl"
const SURFACE_SHADER_PATH = "res://Development/Scripts/Systems/WaterSystem/Core/Shaders/WaterSurface.gdshader"
...
func _bake_obstacles():
	...
	if not world_3d:
		# Only print if we are expecting to be in a world
		if not Engine.is_editor_hint():
			print("[WaterManager] Cannot bake obstacles: No World3D found.")
		return
...
func spawn_debug_test_mover():
	...
	var spawner_script = load("res://Development/Scripts/Systems/WaterSystem/Core/WaterTestSpawner.gd")
	if spawner_script:
		...
		if Engine.is_editor_hint():
			...
			if tree and tree.edited_scene_root:
				spawner.owner = tree.edited_scene_root
