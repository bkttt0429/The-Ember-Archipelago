...
### 2026-01-08: 下一階段架構規劃 - 動態海洋與天氣系統 (Dynamic Ocean & Weather Architecture)

為了建立一個具備「電影感」且「玩法深度」的海洋世界，我們規劃了以下四大核心系統。這些系統將物理環境與視覺表現緊密結合，使玩家的每一次航行都充滿挑戰與沉浸感。

#### 一、 風力系統架構 (Wind Dynamics)
風力不應只是一個數值，而是一個具備方向、強度與空間擾動的向量場。

*   **全域風力單例 (WindManager)**:
    *   **核心參數**: `wind_vector` (Vector3)、`gust_strength` (陣風強度) 與 `wind_noise` (使用 FastNoiseLite 模擬風的脈動)。
    *   **作用範圍**: 作為所有物理物件（船帆、植被、波浪）的唯一真值源。
*   **空間局部風場 (WindArea3D)**:
    *   在特定的島嶼峽谷或海龍捲周邊設置局部風場，用以疊加或覆蓋全域風力。
*   **視覺呈現**:
    *   **風之軌跡**: 利用 `GPUParticles3D` 生成極簡的硬邊線條粒子，沿著風向快速流動，讓「空氣的流動」肉眼可見。

#### 二、 天氣系統規劃 (Weather States)
使用狀態機（State Machine）管理環境參數的漸變，確保從晴天過度到暴風雨時的流暢感。

| 天氣狀態 | 視覺特徵 (Sky & Fog) | 海面狀態 (Sea State) | 影響機制 |
| :--- | :--- | :--- | :--- |
| **微風晴空** | 高能見度、SDFGI 強烈 | 輕微褶皺、透明度高 | 適合遠距導航。 |
| **迷霧深海** | 體積霧密度提高、色調轉綠 | 波浪週期長、平緩湧浪 | 導航困難，需依賴聲納或燈塔。 |
| **星海風暴** | 雲層發光、頻繁閃電、低能見度 | 巨浪 (7m+)、大量硬邊碎浪 | 船隻受損風險高，航行速度減慢。 |
| **靜謐極光** | 天空出現動態帶狀 Shader | 海面如鏡、僅有物體 RTT 漣漪 | 特殊資源生成（如浮游星礦）。 |

#### 三、 海面交互核心邏輯 (Sea Interaction)
這是風力與天氣轉化為玩家體感的關鍵路徑。

*   **物理驅動的波浪 (PM 頻譜)**:
    *   **連動公式**: `WaterManager` 接收風速 $V$，根據 $H_s = 0.21 \cdot V^2/g$ 自動更新葛斯特納波的振幅。
    *   **效果**: 風越大，浪頭越尖銳且越高，這會直接影響 `BuoyantCell` 的起伏劇烈程度。
*   **動態泡沫縮放**:
    *   天氣轉壞時，Shader 中的 `foam_threshold` 自動降低。原本平靜的海面開始出現大量的雙層硬邊泡沫，模擬白沫橫飛的海況。
*   **環境遮蔽與風影 (Wind Shadow)**:
    *   **創意技術**: 利用 RTT 技術，從上方投影大型島嶼的「陰影圖」到海面。
    *   **交互效果**: 處於島嶼背風側的海面，波浪振幅會自動減小，形成「避風港」，這能增加玩家操作的策略性。

#### 四、 創意發想：額外的交互功能 (Creative Features)
為了提升「電影感」與「遊戲性」，以下是與風、水、天氣相關的創意發想：

*   **聲學交互 (Acoustic Wind)**:
    *   **音效連動**: 根據「風速」與「船隻航向」的夾角，動態改變風切聲的頻率。
    *   **沉浸感**: 當船隻全速逆風航行時，玩家能聽到繩索緊繃與風吼的低通濾波音效。
*   **風帆物理與側傾 (Sail Physics)**:
    *   **帆張力**: 船帆不再是靜態動畫。風力向量與帆面法線的點積（Dot Product）決定了船隻的推力與側傾角度。
    *   **操作感**: 風暴中若不收帆，船隻側傾過大會導致「進水」或「翻覆」。
*   **物體表面的「潮濕度」感應**:
    *   **Shader 連動**: 下雨或大浪拍打後，船隻木板的 `ROUGHNESS` 自動調低（變亮）、`METALLIC` 稍微調高。
    *   **硬邊質感**: 水滴在木板上形成細小的圓形幾何反光點。
*   **星海特有的「星塵漂流」**:
    *   **天氣現象**: 強風會捲起海面的螢光浮游生物。
    *   **交互**: 玩家可以透過捕捉風中的螢光方向來判斷風向，這是一種「視覺化導航」機制。
*   **氣壓與魚群高度**:
    *   暴風雨前夕，利用 RTT 在水底渲染大量的魚群陰影往深處移動。這能作為天氣變化的「生物預警」。
