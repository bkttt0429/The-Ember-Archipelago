...
state = 0 # 0=header, 1=skip player, 2=map

for line in lines:
    if line.startswith('[node name="CapsuleTest" type="Node3D"'):
        state = 1
        continue
...
# Resources missing from the .tmp file entirely
...
parts.insert(ext_idx, ext_8.strip())
parts.insert(ext_idx, ext_15.strip())
parts.insert(ext_idx, ext_11.strip())

header_str = "\n".join(parts) + "\n" + sub_resources_missing

full_text = header_str + "\n" + known_block_1 + "\n" + known_block_2 + "\n" + "".join(map_lines)

full_text = full_text.replace('unique_id="', 'unique_id=').replace('"\n', '\n')
full_text = full_text.replace('parent_id_path="PackedInt32Array(682676425, 109460344)"', 'parent_id_path=PackedInt32Array(682676425, 109460344)')
