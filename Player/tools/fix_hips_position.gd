...
## 將動畫的 Hips 位置固定為第一幀的值（消除位移但保留初始高度）
## 用法：Ctrl+Shift+X 執行
...
func _run() -> void:
	...
	for anim_name in ANIM_NAMES:
		_fix_hips_position(lib, anim_name)
...
func _fix_hips_position(lib: AnimationLibrary, anim_name: String) -> void:
	...
	for i in anim.get_track_count():
		...
		if path.contains("Hips") and anim.track_get_type(i) == Animation.TYPE_POSITION_3D:
			...
			print("  First frame Hips: ", first_pos)
			print("  Key count: ", key_count)
			
			# 把所有 key 都設為第一幀的位置（消除動畫中的位移）
			for key_idx in key_count:
				anim.track_set_key_value(i, key_idx, first_pos)
			
			print("  All positions set to first frame value!")
			return
