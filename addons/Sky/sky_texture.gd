...
@export var sun_position = Vector3(0.0, 1.0, 0.0):
	set = set_sun_position,
	get = get_sun_position
@export var night_sky: Texture2D = null:
	set = set_night_sky,
	get = get_night_sky
@export var rotate_night_sky = Basis():
	set = set_rotate_night_sky,
	get = get_rotate_night_sky
...
func set_sun_position(new_position):
	...
	if material:
		material.set_shader_parameter("sun_pos", sun_position)
		_trigger_update_sky()
...
func set_night_sky(new_texture):
	...
	if material:
		material.set_shader_parameter("night_sky", night_sky)
		_trigger_update_sky()
...
func set_rotate_night_sky(new_basis):
	...
	if material:
		...
		material.set_shader_parameter("rotate_night_sky", rotate_night_sky.inverse())
		_trigger_update_sky()
...
func set_time_of_day(hours, directional_light, horizontal_angle = 0.0):
	...
	if directional_light:
		...
		# Fix LookAt error when sun is directly overhead (Noon)
		if abs(sun_position.x) < 0.001 and abs(sun_position.z) < 0.001:
			directional_light.transform = t.looking_at(Vector3(0.0, 0.0, 0.0), Vector3(0.0, 0.0, 1.0))
		else:
