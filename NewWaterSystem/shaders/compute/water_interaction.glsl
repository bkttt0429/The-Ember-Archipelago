...
void main() {
    ...
    ivec2 posD = ivec2(uv.x, min(uv.y + 1, sy));
    
    float hL = imageLoad(tex_in, posL).r;
    float hR = imageLoad(tex_in, posR).r;
    float hU = imageLoad(tex_in, posU).r;
    float hD = imageLoad(tex_in, posD).r;
